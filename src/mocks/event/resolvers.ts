...
import { INSUFFICIENT_DEPOSIT_REWARD, getRole } from '../utils';
...
type EventResolver = {
  ...
  patchEvent: HttpResponseResolver<{ eventId: string }, EventPatchRequest, never>;
...
export const eventResolver: EventResolver = {
  createEvent: async ({ request }) => {
    ...
    if (body.reward >= INSUFFICIENT_DEPOSIT_REWARD) {
      return HttpResponse.json({ message: 'Insufficient deposit' }, { status: 400 });
    }

    return HttpResponse.json(
      { id: 'event-new-001', title: body.title, condition: body.condition, reward: body.reward, isActive: true },
      { status: 201 },
    );
  },
  ...
  patchEvent: async ({ request, params }) => {
    ...
    if (body.isActive === false && !event.isActive) {
      return HttpResponse.json({ message: 'Event already closed' }, { status: 400 });
    }
