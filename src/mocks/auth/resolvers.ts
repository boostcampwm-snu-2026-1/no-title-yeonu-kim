...
export const authResolver: AuthResolver = {
  signUp: async ({ request }) => {
    ...
    if (body.role === 'REVIEWER' && body.email.includes('+fail')) {
      return HttpResponse.json({ message: 'Email verification code expired or invalid' }, { status: 400 });
    }

    // 사장님: 사전 설정된 비밀번호 불일치
    if (body.role === 'OWNER' && body.password === 'wrong-owner-secret') {
      return HttpResponse.json({ message: 'Owner password mismatch' }, { status: 401 });
    }
  ...
  signIn: async ({ request }) => {
    ...
    // 계정 없음 or 비밀번호 불일치
    if (body.mail === 'notfound@example.com' || body.password === 'wrongpassword') {
      return HttpResponse.json({ message: 'Invalid credentials' }, { status: 401 });
    }

    // mail에 'owner' 포함 시 사장님 계정 반환, 그 외 리뷰어
    const response = body.mail.includes('owner') ? mockOwnerUser : mockReviewerUser;
